...
def extract_frames(video_path, output_dir, frame_interval=1, skip_seconds=0, gray=False, csv_file=None):
    ...
    summary_path = os.path.join(output_dir, "extract_info.txt")
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("=== Frame Extraction Summary ===\n")
        ...
        f.write(f"Total extracted: {saved}\n")
        f.write(f"Start time: {timedelta(seconds=int(start_time))}\n")
        f.write(f"End time: {timedelta(seconds=int(end_time))}\n")
        f.write(f"Color mode: {'Grayscale' if gray else 'Color'}\n")
        f.write(f"Output directory: {output_dir}\n")
        f.write(f"CSV file: {csv_file if csv_file else 'None'}\n")
        f.write("Note: Extracted frames include timestamp and frame number watermark\n")
